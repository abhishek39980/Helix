...
Developed for security researchers, law enforcement agencies, and digital forensic analysts, Helix automates the collection and correlation of EXIF metadata, language patterns, social activity schedules, timezone distributions, and image compression mutations.
...
*   **Evidence Chain Integrity**: Automated SQLite schema logging for Cases, Analysis Sessions, and Audit Logs, exportable to court-admissible PDF reports.
...
*   **📷 Advanced Media Forensics**:
    *   Generates cryptographic MD5 hashes and Perceptual Hashes (pHash) to detect tamper history.
...
graph TD
    ...
    B -->|httpx / async requests| F[External OSINT APIs / Local LLM]
...
### Technical Breakdown:
*   **Frontend**: Built with React 18, Vite 5, and Tailwind CSS. State management handles session recovery (`localStorage`) and asynchronous XHR progress hooks. Maps are rendered client-side using Leaflet.
...
│   ├── backend.py                # FastAPI routes, middleware, and geolocation orchestrator
│   ├── db.py                     # SQLAlchemy Database models (Case, Session, AuditLog)
...
├── frontend/
│   ├── src/
│   │   ├── components/           # Modular React layout, tabs, and map components
...
Helix contains a comprehensive test suite covering API endpoints, case CRUD logic, async profile fetching, and security mechanisms.
...
*   **Input Sanitization**: File uploads are restricted to specific allowed extensions (MP4, PNG, JPG) and files are validated up to a strict 100MB limit. MD5/pHash generation verifies file contents.
